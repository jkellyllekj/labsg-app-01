...
app.get("/", (req, res) => {
  ...
  const HOME_JS_RENDER_GLUE = `
      ...
      function dolphinAnimFinish(mainEl, extraEls, activeBtn) {
        ...
        const FADE_MS = 100;           // cross-fade chunk
        const SPLASH_HOLD_MS = 500;    // keep splash visible
        const IDLE_FADEIN_MS = 150;    // dolphin fade back in
        ...
        __dolphinAnimTimers.push(setTimeout(() => {
          ...
          __dolphinAnimTimers.push(setTimeout(() => {
            ...
            for (const el of all) {
              ...
              el.style.transition = "opacity " + FADE_MS + "ms linear, transform " + FADE_MS + "ms ease-out";
              el.style.opacity = "0";
              el.style.transform = "rotate(-120deg) scale(0.7)";
              void el.offsetWidth;
              el.style.opacity = "1";
              el.style.transform = "rotate(-120deg) scale(1.15)";
            }
            ...
            __dolphinAnimTimers.push(setTimeout(() => {
              ...
              for (const el of all) {
                ...
                el.style.transform = "rotate(-120deg) scale(1)";
              }
            }, FADE_MS + 20));
            ...
            __dolphinAnimTimers.push(setTimeout(() => {
              ...
              for (const el of all) {
                el.style.transition = "opacity " + FADE_MS + "ms linear, transform " + FADE_MS + "ms ease-out";
                el.style.opacity = "0";
                el.style.transform = "rotate(-120deg) scale(0.9)";
              }
              ...
              __dolphinAnimTimers.push(setTimeout(() => {
                ...
                for (const el of all) {
                  el.textContent = "🐬";
                  el.style.transition = "opacity " + IDLE_FADEIN_MS + "ms linear";
                  el.style.opacity = "0";
                  el.style.transform = "rotate(0deg) scale(1)";
                  void el.offsetWidth;
                  el.style.opacity = "1";
...
app.post("/generate-workout", (req, res) => {
  ...
  function buildWorkout({ targetTotal, poolLen, unitsShort, poolLabel, thresholdPace, opts, seed }) {
    ...
    for (const s of sets) {
      ...
      const body = buildOneSetBodyShared({
        label: setLabel,
